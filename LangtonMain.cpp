 ...
 */
bool run_simulation(size_t indent) {
    size_t length = menu::prompt_range("board length", 1, MAX_LENGTH, indent);
    size_t width = menu::prompt_range("board width", 1, MAX_WIDTH, indent);
    size_t row = menu::prompt_range("starting row", 0ul, length - 1, indent);
    size_t col = menu::prompt_range("starting column", 0ul, width - 1, indent);
    const std::vector<std::string> options = {"NORTH", "EAST", "SOUTH", "WEST"};
    size_t direction = menu::prompt_option("starting direction", options, indent) - 1;
    size_t iterations = menu::prompt_range("number of iterations", 1, MAX_ITERATIONS, indent);
